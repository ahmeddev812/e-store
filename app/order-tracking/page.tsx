  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 animate-fade-in">
      <div className="mb-8 text-center">
        ...
        <p className="text-white/50 text-sm">Enter your order ID to check the current status.</p>
      </div>

      <Card className="mb-8 !bg-white/5 !backdrop-blur-xl">
      ...
      {error && (
        <Card className="!bg-white/5 !backdrop-blur-xl">
          <CardContent className="p-8 flex flex-col items-center text-center">
            ...
            <p className="text-white font-medium mb-1">Order Not Found</p>
        ...
        <div className="space-y-6">
                      ...
                      <div key={step} className="flex items-start gap-4">
                        <div
                          className={`relative z-10 rounded-full p-2 transition-all duration-300 ${
                            isComplete ? "bg-[#F57224] text-white shadow-[0_0_15px_rgba(245,114,36,0.5)]" : "bg-white/5 backdrop-blur-xl text-white/30"
                  ...
                  return (
                    <div key={item.productId} className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white/50 text-xs">{formatUSD(dp)} x {item.quantity}</p>
                      </div>
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <p className="text-white/80 text-sm">{order.shipping.name}</p>
              <p className="text-white/50 text-sm">{order.shipping.address}</p>
      ...
      {!order && !error && !searched && (
        <div className="text-center py-12 animate-fade-in">
          <div className="inline-flex rounded-full bg-white/5 backdrop-blur-xl border border-white/10 p-6 mb-4">
            <Truck className="size-12 text-white/20" />
          </div>
          <p className="text-white/40 text-sm">Enter an order ID above to track your package.</p>
        </div>
      )}
    </div>
  )
